...
st.set_page_config(page_title="GovernX AI | Dashboard", page_icon="📊", layout="wide")
if CSS.exists():
...
def filter_data(df):
    with st.sidebar:
        ...
        search = st.text_input("⌕ Search company", placeholder="e.g. Reliance Industries")
        ...
        selected = st.multiselect("Risk level", levels, default=levels, placeholder="Choose options")
...
for col, (icon, value, label, theme) in zip(cards, items):
...
risk = filtered["Risk_Level"].value_counts().reindex(risk_order, fill_value=0).reset_index()
risk.columns = ["Risk Level", "Companies"]
...
with b:
    ...
    dist = pd.cut(complete["Governance_Score"], bins=bins, labels=labels, include_lowest=True).value_counts().reindex(labels, fill_value=0).reset_index()
    dist.columns = ["Range", "Companies"]
    fig = px.bar(dist, x="Range", y="Companies", text="Companies", title="Governance score distribution")
    fig.update_traces(textposition="outside")
    fig.update_layout(margin=dict(l=8, r=8, t=46, b=8), paper_bgcolor="white")
    st.plotly_chart(fig, use_container_width=True)
with c:
    if "Industry" in complete and len(complete):
...
with q:
    ...
    for name, col, max_score in pillar_specs:
        if col in complete and complete[col].notna().any():
            names.append(name)
            vals.append(float(complete[col].mean()) / max_score * 100)
    if vals:
...
d1, d2 = st.columns(2)
with d1:
    st.download_button("⬇ Download CSV", filtered.to_csv(index=False).encode("utf-8"), "governx_dashboard.csv", "text/csv", use_container_width=True)
with d2:
    st.download_button("▥ Download Excel", excel_bytes(filtered), "governx_dashboard.xlsx", "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", use_container_width=True)
